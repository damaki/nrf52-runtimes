...
with NRF52_Runtime_Config; use NRF52_Runtime_Config;
...
package body System.BB.Board_Support is
   ...
   Alarm_Interrupt_ID : constant Interrupt_ID :=
     (case NRF52_Runtime_Config.Time_Base is
        when RTC0 => 11,  --  RTC0 IRQ
        when RTC1 => 17,  --  RTC1 IRQ
        when RTC2 => 36); --  RTC2 IRQ

   RTC_Periph : aliased RTC_Peripheral with
     Import,
     Address => (case NRF52_Runtime_Config.Time_Base is
                   when RTC0 => RTC0_Base,
                   when RTC1 => RTC1_Base,
                   when RTC2 => RTC2_Base);
   ...
   --  One of the RTCs is used as the clock source, which we use to implement
   --  "tick-less" alarm handling. Which RTC is used is set in the Alire
   --  crate configuration.
   --
   ...
   begin
      ...
      --  event.
      RTC_Periph.CC (0).COMPARE := 0;

      --  Clear RTC
      RTC_Periph.TASKS_CLEAR := (TASKS_CLEAR => 1, others => <>);

      --  Run at 32.768 kHz
      RTC_Periph.PRESCALER.PRESCALER := 0;

      --  Enable CC[0] interrupt only; TICK and OVRFLW aren't needed.
      RTC_Periph.INTENSET.TICK    := Intenset_Tick_Field_Reset;
      RTC_Periph.INTENSET.OVRFLW  := Intenset_Ovrflw_Field_Reset;
      RTC_Periph.INTENSET.COMPARE := (As_Array => False, --  Use COMPARE0
                                      Val      => 2#0001#);
   ...
   package body Time is
      ...
      begin
         ...
         RTC_Periph.EVENTS_COMPARE (0) := (EVENTS_COMPARE => 0, others => <>);
      ...
      begin
         ...
         RTC_Counter                := RTC_Periph.COUNTER.COUNTER;
         CC0_Value                  := RTC_Counter + RTC_Ticks;
         RTC_Periph.CC (0).COMPARE := CC0_Value;
